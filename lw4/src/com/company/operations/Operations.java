...
public class Operations {
    ...
    public static BigNumber sqrt(BigNumber number) {
        return Sqrt.calc(number);
    }
    ...
    public static int compare(BigNumber number1, BigNumber number2) {
        number1 = removeExtraZeros(number1);
        number2 = removeExtraZeros(number2);
    ...
    public static BigNumber removeExtraZeros(BigNumber bigNumber) {
        int zeroAmount = 0;
        for (int i = 0; i < bigNumber.getAmount(); ++i) {
            if (bigNumber.getDigit(i) == 0)
                ++zeroAmount;
            else
                break;
        }
        List<Integer> result = bigNumber.get().subList(zeroAmount, bigNumber.getAmount());
